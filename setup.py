...
# LADDER only supports Linux platform
assert sys.platform.startswith("linux"), "LADDER only supports Linux platform (including WSL)."
...
def get_ladder_version(with_cuda=True, with_system_info=True) -> str:
    version = find_version(get_path("python/ladder", "__init__.py"))
    if with_system_info:
...
class LADDERInstallCommand(install):
...
class LADDERBuilPydCommand(build_py):
    """Customized setuptools install command - builds TVM after setting up LLVM."""
...
setup(
    ...
    url="https://github.com/microsoft/LADDER",
    classifiers=[
    ...
    cmdclass={
        "install": LADDERInstallCommand,
        "build_py": LADDERBuilPydCommand,
        "sdist": LADDERSdistCommand,
    },
)
