...
# fmt: off
def _tir_f32x2_to_bf16x2_to_u32(v0: tir.PrimExpr,
                                v1: tir.PrimExpr,
    ...
    for data in [v0, v1]:
        ...
        if round_to_even:
            rounding_bias = ((u32_val >> tir.const(16, "uint32")) & tir.const(
                1, "uint32")) + tir.const(0x7FFF, "uint32")
...
def _tir_u32_to_bf16x2_to_f32x2(x: tir.PrimExpr):
    ...
    x0 = x & mask
    x1 = (x >> 16) & mask
    return (tir.reinterpret("float32", x << tir.const(16, "uint32"))
            for x in [x0, x1])


def _tir_u32_to_int_to_float(nbit: int, val: tir.PrimExpr, pos: tir.PrimExpr,
                             dtype: str):
...
def _tir_packed_uint_to_uint_to_float(storage_nbit: int):
    ...
    def f_convert(nbit: int, val: tir.PrimExpr, pos: tir.PrimExpr, dtype: str):
        ...
        return (
            (val >>
             (pos.astype("uint32") * tir.const(nbit, "uint32"))) & tir.const(
                 (1 << nbit) - 1, "uint32")).astype(dtype) - tir.const(
                     max_int_value, dtype)
...
def _tir_packed_int_to_int_to_float(storage_nbit: int):
    ...
    def f_convert(nbit: int, val: tir.PrimExpr, pos: tir.PrimExpr, dtype: str):
        ...
        unextended = (val >>
                      (pos.astype("int32") * tir.const(nbit, "int32"))) & mask
        return tir.Cast(dtype, (unextended << tir.const(32 - nbit, "int32")) >>
...
def _tir_f32_to_uint_to_f4(val: tir.PrimExpr):
    ...
    e_f4 = tir.Select(
        ...
        tir.Min(e_f32 - tir.const(120, "uint32") + m_h, tir.const(7,
                                                                  "uint32")),
...
def _tir_f16_to_uint_to_f4(val: tir.PrimExpr):
    ...
    e_f4 = tir.Select(
        ...
        tir.Select(e_f16 == tir.const(8, "uint32"), tir.const(1, "uint32"),
                   tir.const(0, "uint32")))
    return (s << tir.const(3, "uint32")) | e_f4


def _tir_u32_to_f4_to_f32(nbit: int, val: tir.PrimExpr, pos: tir.PrimExpr,
                          dtype: str):
    assert nbit == 4
    ...
    val_f32 = tir.reinterpret(
        "float32",
        (e_f32 | (s << tir.const(8, "uint32"))) << tir.const(23, "uint32"))
    return tir.Select(e_f4 == tir.const(0, "uint32"), tir.const(0, "float32"),
                      val_f32)


def _tir_u32_to_f4_to_f16(nbit: int, val: tir.PrimExpr, pos: tir.PrimExpr,
                          dtype: str):
    assert nbit == 4
    ...
    return tir.Select(e_f4 == tir.const(0, "uint32"), tir.const(0, "float16"),
                      val_f16)
...
def _tir_packed_to_unsigned_convert(storage_type="uint", storage_nbit=8):
    ...
    def f_convert(nbit: int, val: tvm.tir.PrimExpr, pos: tvm.tir.PrimExpr,
                  dtype: str):
        assert val.dtype == storage_dtype, f"{val.dtype} != {storage_dtype}"
        mask = tvm.tir.const((1 << nbit) - 1, storage_dtype)
        return ((val >>
                 (pos * nbit).astype(storage_dtype)) & mask).astype(dtype)
