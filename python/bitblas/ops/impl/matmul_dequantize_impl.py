...
def matmul_nt_dequantize_b(
    ...
    B = te.placeholder((N, K // storage_nbit * bit),
                       name="B",
                       dtype=storage_dtype)
    ...
    def decode_func(n, k):
        if source_format == "uint":
            ...
            w = LUT[_tir_packed_to_unsigned_convert(
            ...
            raise ValueError(
                "Unsupported source_format: {}".format(source_format))
    ...
    C = te.compute(
        (M, N),
        lambda i, j: te.sum(A[i, k].astype(accum_dtype) * B_decode[j, k].
                            astype(accum_dtype),
                            axis=k),
        name="C",
    )
...
def matmul_nt_dequantize_b_propagate_b(
    ...
    _, inverse_indexmap = get_propagate_map(trans=True,
                                            dtype=in_dtype,
                                            matrix_name="B")
    target_dtype = DataType(in_dtype)
    scaling_factor = 1
    if bit > 0 and bit < target_dtype.bits:
        scaling_factor = ((target_dtype.bits // bit) *
    ...
    A = te.placeholder((M, K), name="A", dtype=in_dtype)
    B = te.placeholder((N // l, (K // scaling_factor) // qr, l, qr),
                       name="B",
                       dtype=storage_dtype)
    LUT = te.placeholder((1 << bit, ), name="LUT", dtype=in_dtype)
    Scale = te.placeholder((N, K // group_size), name="Scale", dtype=in_dtype)
...
def matmul_nt_dequantize_b_propagate_a_propagate_b(
    ...
    C = te.compute(
        (M, N),
        lambda i, j: te.sum(
            A_reindex[i, k].astype(accum_dtype) * B_decode[j, k].astype(
                accum_dtype),
            axis=k,
        ),
        name="C",
