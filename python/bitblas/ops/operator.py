...
class Operator(ABC):
    ...
    def _build_runtime_module(self, target: Target):
        ...
        if self.arch.platform == "CUDA":
            ...
            try:
                ...
                with tvm.transform.PassContext(config={
                    ...
                    rt_mod = tvm.build(self.optimized_func,
                                       target=target,
                                       name=self.name)
        ...
        if rt_mod:
            ...
            self.time_evaluator = rt_mod.time_evaluator(rt_mod.entry_name,
                                                        self.arch.device,
                                                        number=10)
            self.function_handle = rt_mod.get_function(
                rt_mod.entry_name).handle
                ...
                try:
                    if (self.dynamic_range is not None
                        ...
                        wrapper = CUDASourceWrapperWithDynamic(
                            self.optimized_func, self.get_source(target),
                            self.arch)
                    else:
                        wrapper = CUDASourceWrapper(self.optimized_func,
                    ...
                    logger.debug("Failed to build runtime library {}".format(
                        build_runtime_library_error))

        return rt_mod

    def apply_default_schedule(self, func_mod: IRModule,
                               target: Target) -> IRModule:
    ...
    def _forward_from_tvm_args(self, *args):
        _tvm_args = [
            self._tensor_adapter(arg, self.arch.device) for arg in args
        ]
    ...
    def _forward_from_prebuild_lib(self, *args):
        ctypes_args = [
            ctypes.c_void_p(arr.data_ptr())
            if not isinstance(arr, int) else arr for arr in args
        ]
