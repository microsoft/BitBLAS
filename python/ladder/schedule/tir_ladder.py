...
class TIRLadderMMAScheduler4D(TIRSchedulerBase):
    ...
    def schedule_consistent_sm70(self):
        ...
        # ------------------------ Tensorize and Pipelining -------------------------
        init_intrin = WMMA_FILL_16x16x16_F16_INTRIN
        load_a_intrin = WMMA_LOAD_16x16x16_F16_A_INTRIN
        load_b_intrin = WMMA_LOAD_16x16x16_F16_B_INTRIN
        load_b_intrin_trans = WMMA_LOAD_16x16x16_F16_B_TRANS_INTRIN
        compute_intrin = WMMA_SYNC_16x16x16_f16f16f16_INTRIN
        compute_trans_intrin = WMMA_SYNC_16x16x16_f16f16f16_TRANS_INTRIN
        store_intrin = WMMA_STORE_16x16x16_F16_SHARED_INTRIN
        init_block_b = sch.decompose_reduction(C, ko)
        write_sch(sch, "decompose_reduction")
        init_block_b_loops = sch.get_loops(init_block_b)
        init_block_b_i, init_block_b_j = sch.get_loops(init_block_b)[-4:-2]
    ...
    def schedule_inconsistent_sm70(self, is_a_consistent=False, is_b_consistent=False):
        from tvm.tir.tensor_intrin.cuda import (
            WMMA_FILL_16x16x16_F16_INTRIN,
            WMMA_LOAD_16x16x16_F16_A_INTRIN,
            WMMA_LOAD_16x16x16_F16_B_INTRIN,
    ...
    def schedule(self) -> tir.Schedule:
        ...
        if self.config.use_tc == "70":
            ...
            if is_consistent:
                return self.schedule_consistent_sm70()
            else:
                return self.schedule_inconsistent_sm70()
