...
# the topi layout_transform_inverse compute does not simplify well when lowering, so we implement a new one here
...
def rel_layout_transform_inverse(arg_types, attrs):
...
def op_compute(attrs, inputs, output_type):
    ...
    if is_inverse:
        ...
        def fcompute(*args):
            ...
            return inputs[0][new_index]
    out = te.compute(out_shape, fcompute, "ladder_layout_transform_inverse")
    return [out]

@target.override_native_generic_func("strategy_ladder_layout_transform_inverse")
def op_strategy(attrs, inputs, out_type, target):
    strategy = relay.op.OpStrategy()
    strategy.add_implementation(
        op_compute,
        wrap_topi_schedule(topi.generic.schedule_extern),
        name="ladder.layout_transform_inverse.generic",
    )
    return strategy

def register_layout_transform_inverse():
    op_name = "ladder.layout_transform_inverse"
    reg.register(op_name, "Transform the Layout.")
    op = reg.get(op_name)
    op.set_num_inputs(1)
    op.set_support_level(10)
    op.add_type_rel(op_name + "_rel", rel_layout_transform_inverse)
...
register_layout_transform_inverse()
