...
class MatmulFineGrainSIMTScheduler(MatmulSIMTBaseScheduler):
    ...
    block_size_x: int = 2
    block_size_y: int = 2
    thread_row_tiles: int = 32
    thread_col_tiles: int = 32
            ...
            with T.Kernel(
                ...
                for ko in T.serial(K // block_K):
                    ...
                    for ki in T.serial((block_K // micro_size_k)):
                        ...
                        for i, j, mk in T.grid(local_size_a, local_size_b, micro_size_k):
                            C_local[i * local_size_b + j] += A_local[i, mk] * B_local[j, mk]
