...
__global__ void __launch_bounds__(128) bitblas_kernel_fp16_int2_fp16_m128n15360k5120_nt(half* __restrict__ A, half* __restrict__ QB, half* __restrict__ D) {
  ...
  __shared__ half B_zeros_reindex_shared[3072];
  signed char B_local[4];
  half B_zeros_reindex_local[8];
  half A_reindex_shared_warp[16];
  half B_zeros_reindex_shared_warp[48];
...
;
    ...
    for (int ax3_0_0 = 0; ax3_0_0 < 160; ++ax3_0_0) {
      ...
      for (int ax1_ax2_0_fused_0 = 0; ax1_ax2_0_fused_0 < 3; ++ax1_ax2_0_fused_0) {
        *(int*)(B_local + 0) = *(int*)(B + ((((((((int)blockIdx.x) * 245760) + (ax1_ax2_0_fused_0 * 81920)) + (((int)threadIdx.y) * 20480)) + ((((int)threadIdx.x) >> 2) * 2560)) + (ax3_0_0 * 16)) + ((((int)threadIdx.x) & 3) * 4)));
...
*((uint *)&(&(B_zeros_reindex_shared[(((int)threadIdx.y) * 512)]))[((((((local_id % 4) / 2) * 8) + (threadIdx.x / 4)) * 16) + ((((local_id / 4) * 8) + ((threadIdx.x % 4) * 2)) + (local_id % 2)))]) = *((uint *)&C_reindex_shared_warp[((ax0 * 48) + (ax1 * 8)) + local_id]);
}
;
        __syncthreads();
        #pragma unroll
        for (int ax0_ax1_ax2_ax3_ax4_fused_0 = 0; ax0_ax1_ax2_ax3_ax4_fused_0 < 1; ++ax0_ax1_ax2_ax3_ax4_fused_0) {
          *(uint4*)(D + ((((((((int)threadIdx.y) * 491520) + (ax0 * 245760)) + ((((int)threadIdx.x) >> 1) * 15360)) + (((int)blockIdx.x) * 96)) + (ax1 * 16)) + ((((int)threadIdx.x) & 1) * 8))) = *(uint4*)(B_zeros_reindex_shared + ((((int)threadIdx.y) * 512) + (((int)threadIdx.x) * 8)));
        }
      }
    }
  }
