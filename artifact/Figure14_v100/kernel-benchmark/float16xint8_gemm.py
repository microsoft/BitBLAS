...
for M, N, K in shapes:
    ...
    def _tir_u8_to_int_to_float(nbit: int, val: tvm.tir.PrimExpr, pos: tvm.tir.PrimExpr, dtype: str):
        ...
        return T.Cast(dtype, (val >> (pos * nbit).astype("int8")) & mask)
    ...
    def ladder_gemm(M, N, K, wmma_m, wmma_n, wmma_k):
        ...
        def decode_func(n, k, nn, kk):
            w = _tir_u8_to_int_to_float(bit, B[n, k, nn, kk // n_float_per_i8], kk % n_float_per_i8, "float16")
            return w

        B_decode = te.compute(
            (N // wmma_n, K // wmma_k, wmma_n, wmma_k),
            lambda n, k, nn, kk: B[n, k, nn, kk].astype('float16'),
            name='B_decode'
        )
