...
parser.add_argument('--convert_mxfp', action="store_true")
...
def run(prefix, arch, async_propagate, fake_quant, quant_config, convert_int):
    ...
    if fake_quant > -1:
        mod = ladder.relay.transform.LadderFakeQuant(quant_type=fake_quant, quant_config=quant_config, convert_int=convert_int, convert_mxfp=args.convert_mxfp, enable_perfect=False)(mod)
    ...
    if not (args.convert_int or args.convert_mxfp):
    ...
    if args.convert_mxfp:
        mod = ladder.relay.transform.AnnotateFastDecoding(
            fast_decoding=False, consistent_config=(False, False)
        )(mod)
