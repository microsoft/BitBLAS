...
def ladder_conv_nhwc_hwnc(n, f, h, w, c, kh, kw, s, d, p, warp_i = 16, warp_j = 16, warp_k = 16):
    ...
    B = te.placeholder((f // warp_j, kh*kw*c // warp_k, warp_j, warp_k), name='weight', dtype='float16')
    ...
    C = te.compute(
            ...
            lambda i, j, ii, jj: te.sum(data[i, k_axis, ii, wk_axis].astype('float16') * B[j, k_axis, jj, wk_axis].astype('float16'), axis=[k_axis, wk_axis]),
            "T_conv",
        )
    return A, B, C
