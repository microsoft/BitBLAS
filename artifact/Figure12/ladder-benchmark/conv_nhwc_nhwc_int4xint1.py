...
def ladder_conv_nhwc_hwnc(n, f, h, w, c, kh, kw, s, d, p, warp_i = 16, warp_j = 16, warp_k = 32):
    def _tir_u8_to_2u4_u8(nbit: int, val: tvm.tir.PrimExpr, pos: tvm.tir.PrimExpr, dtype: str):
    ...
    B = te.placeholder((f // warp_j, kh*kw*(c//2) // warp_k, warp_j, warp_k // 4 * bit), name='weight', dtype='int8')
    
    def B_decode_func(n, k, nn, kk):
        w = _tir_u8_to_2u4_u8(bit, B[n, k, nn, kk // n_float_per_i8], (kk * 2) % n_float_per_i8, "int8")
        return w

    B_decode = te.compute(
        (f // warp_j, kh*kw*(c//2) // warp_k, warp_j, warp_k),
        B_decode_func,
        name='B_decode'
    )
    ...
    C = te.compute(
            ...
            lambda i, j, ii, jj: te.sum(data[i, k_axis, ii, wk_axis].astype('int32') * B_decode[j, k_axis, jj, wk_axis].astype('int32'), axis=[k_axis, wk_axis]),
            "T_conv",
        )
    return A, B, C

def cast(_N, _H, _W, _C, wmma_m, wmma_n):
    M = _N * _H * _W
    N = _C
    C = te.placeholder((M // wmma_m, N // wmma_n, wmma_m, wmma_n), name='C', dtype='int32')
...
shapes = [
    [128, 64, 56, 56, 64, 3, 3, 1, 1, 0],
    [128, 64, 56, 56, 64, 1, 1, 1, 1, 0],
    [128, 128, 28, 28, 128, 3, 3, 1, 1, 0],
    [128, 512, 28, 28, 128, 1, 1, 1, 1, 0],
]
# shapes = mobile_net
perf_map = []
