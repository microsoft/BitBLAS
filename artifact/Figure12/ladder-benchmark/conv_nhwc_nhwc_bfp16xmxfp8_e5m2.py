...
def ladder_conv_nhwc_hwnc(n, f, h, w, c, kh, kw, s, d, p, warp_i = 16, warp_j = 16, warp_k = 16, dtype="float16"):
    ...
    def B_decode_func(n, k, nn, kk):
        w = _tir_u8_to_f8_to_float(bit, B[n, k, nn, kk // n_float_per_i8], kk % n_float_per_i8, "float16", Scales[(k * warp_k + kk) // group_size, n * warp_j + nn])
        return w
...
for n, c, h, w, f, kh, kw, s, d, p in shapes:
    ...
    if n % 16 != 0 or f % 16 != 0 or c % 16 != 0:
        performs_map[key] = 10000.0
        continue
    oh = (h + 2 * p - kh) // s + 1
    ow = (w + 2 * p - kw) // s + 1
    ...
    try:
        ...
        performs_map.append((key, 10000.0))
        continue
    best_latency = 10000
    best = None
    values = []
    ...
    performs_map.append((key, best_latency))


for key, latency in performs_map:
