...
if [ ! -f "./logs/resnet-50-b128-(128, 3, 224, 224)" ] || [ $force_tune -eq 1 ]; then
    START_TIME=$(date +%s)
    python3 tune_from_onnx.py --workload resnet-50 --batch 128 --trials 20000 2>&1 | tee resnet-50-b128.log
    END_TIME=$(date +%s)
...
if [ ! -f "./logs/shufflenet-b128-(128, 3, 224, 224)" ] || [ $force_tune -eq 1 ]; then
    START_TIME=$(date +%s)
    python3 tune_from_onnx.py --workload shufflenet --batch 128 --trials 20000 2>&1 | tee shufflenet-b128.log
    END_TIME=$(date +%s)
