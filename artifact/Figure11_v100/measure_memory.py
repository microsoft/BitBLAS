...
CHECKPOINT_PATH = os.path.join(os.getcwd(), "../checkpoints/Figure11_v100")
...
def vllm_inference(model='llama', batch_size=1, seq_len=1):
    ...
    target_process = subprocess.Popen(f'cd {pwd}/vllm-benchmark; PYTHONPATH={PYTHONPATH} python {run_file}  --batch_size {batch_size} --seq_length {seq_len}; cd ..', shell=True)
    return target_process

def vllm_fp16_int4_inference(model='llama', batch_size=1, seq_len=1):
    run_file = 'benchmark_llama.py' if model == 'llama' else 'benchmark_bloom.py'
    target_process = subprocess.Popen(f'cd {pwd}/vllm-benchmark; python {run_file}  --batch_size {batch_size} --seq_length {seq_len}; cd ..', shell=True)
    return target_process
...
def ladder_fp16_int4_inference(model='llama', batch_size=1, seq_len=1):
    run_file = 'ladder_with_fake_dense_dequantize.py'
    if model=='llama':
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/llama2-70b/ladder_with_fake_dense_dequantize_fq_0_int_4_-1_bs{batch_size}_seq{seq_len}_ci_False'
    else:
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/bloom-176b/ladder_with_fake_dense_dequantize_fq_0_int_4_-1_bs{batch_size}_seq{seq_len}_ci_False'
    target_process = subprocess.Popen(f'cd {pwd}/ladder-benchmark; PYTHONPATH={PYTHONPATH} CPLUS_INCLUDE_PATH={CPLUS_INCLUDE_PATH} python {run_file} --prebuilt_path {model_file} ; cd ..', shell=True)
    return target_process
...
def ladder_fp16_nf4_inference(model='llama', batch_size=1, seq_len=1):
    run_file = 'ladder_with_fake_dense_dequantize.py'
    if model=='llama':
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/llama2-70b/ladder_with_fake_dense_dequantize_fq_0_nf_4_-1_bs{batch_size}_seq{seq_len}_ci_False'
    else:
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/bloom-176b/ladder_with_fake_dense_dequantize_fq_0_nf_4_-1_bs{batch_size}_seq{seq_len}_ci_False'
    target_process = subprocess.Popen(f'cd {pwd}/ladder-benchmark; PYTHONPATH={PYTHONPATH} CPLUS_INCLUDE_PATH={CPLUS_INCLUDE_PATH} python {run_file} --prebuilt_path {model_file} ; cd ..', shell=True)
    return target_process
...
def ladder_fp8_fp8_inference(model='llama', batch_size=1, seq_len=1):
    run_file = 'ladder_with_fake_dense_dequantize.py'
    if model=='llama':
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/llama2-70b/ladder_with_fake_dense_dequantize_fq_0_fp_e5m2_8_-1_bs{batch_size}_seq{seq_len}_ci_False'
    else:
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/bloom-176b/ladder_with_fake_dense_dequantize_fq_0_fp_e5m2_8_-1_bs{batch_size}_seq{seq_len}_ci_False'
    target_process = subprocess.Popen(f'cd {pwd}/ladder-benchmark; PYTHONPATH={PYTHONPATH} CPLUS_INCLUDE_PATH={CPLUS_INCLUDE_PATH} python {run_file} --prebuilt_path {model_file} ; cd ..', shell=True)
    return target_process
...
def ladder_mxfp8_mxfp8_inference(model='llama', batch_size=1, seq_len=1):
    run_file = 'ladder_with_fake_dense_dequantize.py'
    if model=='llama':
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/llama2-70b/ladder_with_fake_dense_dequantize_fq_0_mxfp_8_-1_bs{batch_size}_seq{seq_len}_ci_False'
        if seq_len in [32, 4096]:
            model_file += '_cf_True'
    else:
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/bloom-176b/ladder_with_fake_dense_dequantize_fq_0_mxfp_8_-1_bs{batch_size}_seq{seq_len}_ci_False'
        if seq_len in [32, 4096]:
            model_file += '_cf_True'
    target_process = subprocess.Popen(f'cd {pwd}/ladder-benchmark; PYTHONPATH={PYTHONPATH} CPLUS_INCLUDE_PATH={CPLUS_INCLUDE_PATH} python {run_file} --prebuilt_path {model_file} ; cd ..', shell=True)
    return target_process
...
def ladder_int8_int1_inference(model='llama', batch_size=1, seq_len=1):
    run_file = 'ladder_with_fake_dense_dequantize.py'
    if model=='llama':
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/llama2-70b/ladder_with_fake_dense_dequantize_fq_0_int_1_-1_bs{batch_size}_seq{seq_len}_ci_True'
    else:
        model_file = f'{CHECKPOINT_PATH}/ladder/checkpoints/bloom-176b/ladder_with_fake_dense_dequantize_fq_0_int_1_-1_bs{batch_size}_seq{seq_len}_ci_True'
    target_process = subprocess.Popen(f'cd {pwd}/ladder-benchmark; PYTHONPATH={PYTHONPATH} CPLUS_INCLUDE_PATH={CPLUS_INCLUDE_PATH} python {run_file} --prebuilt_path {model_file} ; cd ..', shell=True)
    return target_process
