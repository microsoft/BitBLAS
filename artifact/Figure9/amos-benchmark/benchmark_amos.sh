...
RESENET_B128_LOG_PATH="$CHECKPOINT_PATH/amos/logs/resnet50_b128.log"
...
if [ -f "$SHUFFLENET_B128_LOG_PATH" ] && [ $force_tune -eq 0 ]; then
    ...
    cp "$SHUFFLENET_B128_LOG_PATH" "./logs/shufflenet_v2_b128.log"
else
    echo "Running benchmark..."
    python3 ./mapping_shufflenet_tensorcore.py --batch 1 --dtype float16 --trials 20 | tee logs/shufflenet_v2_b128.log
fi


RESENET_B1_LOG_PATH="$CHECKPOINT_PATH/amos/logs/resnet50_b1.log"
...
if [ -f "$SHUFFLENET_B1_LOG_PATH" ] && [ $force_tune -eq 0 ]; then
    ...
    cp "$SHUFFLENET_B1_LOG_PATH" "./logs/shufflenet_v2_b1.log"
else
    echo "Running benchmark..."
    python3 ./mapping_shufflenet_tensorcore.py --batch 1 --dtype float16 --trials 20 | tee logs/shufflenet_v2_b1.log
fi
