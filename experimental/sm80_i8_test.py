...
def gemm(n, m, k):
    ...
    B = te.placeholder((m, k), dtype="int8", name='b')
    K = te.reduce_axis((0, k))
    C = te.compute((n, m), lambda i, j: te.sum(A[i,K].astype("int") * B[j,K].astype("int"), axis=[K]), name='output')
    return A, B, C
...
def sche_gemm(sch: tvm.tir.Schedule):
    ...
    grid_M, tm, block_M = sch.split(ax_M, factors=[None, 8, block_size_M])
    ...
    layoutA = RowMajorTensorOpMultiplicandCrosswise(block_size_M, chunk_size, 8, 64)
    layoutB = ColumnMajorTensorOpMultiplicandCrosswise(block_size_N, chunk_size, 8, 64)
    ...
    cls_code = register_cutlass_i8_warp_mma(warp_size_M, warp_size_N, chunk_size, layoutA, layoutB)
    ...
    sch.tensorize(sch.get_loops(block_init_c)[-2],
        register_cutlass_warp_init_intrin(warp_size_M, warp_size_N, "int32",
        cls_code, block_size_M // warp_size_M, block_size_N // warp_size_N)
    )
    sch.tensorize(sch.get_loops(C)[-3],
        register_gemm_intrin(
            warp_size_M, warp_size_N, chunk_size, "int8", "int32", False, True, layoutA, layoutB)
    )
