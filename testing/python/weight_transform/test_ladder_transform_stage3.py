...
def compare_propagate_with_torch_iter_4_fp16(M, N, inner_m=16, inner_n=16):
    ...
    torch_transformed_b = torch.zeros((M // inner_m, N // inner_n, inner_m, inner_n), dtype=torch.float16)
...
def compare_propagate_with_torch_iter_4_int8(M, N, inner_m=16, inner_n=32):
    ...
    torch_transformed_b = torch.zeros((M // inner_m, N // inner_n, inner_m, inner_n), dtype=torch.int8)
    for i in range(M // inner_m):
        for j in range(N // inner_n):
            for ii in range(inner_m):
                for jj in range(inner_n):
                    ...
                    new_dummy_ii, new_dummy_jj = shared_32x16_to_mma_32x16_layout(dummy_ii, dummy_jj)
