...
@pytest.mark.parametrize(
    ...
    [
        (1, 1024, 1024, "float16", "float16", "float16", 4, "int8", "uint",
         False, False, -1, False, False, False, False, "nt"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (1, 1024, 1024, "float16", "float16", "float16", 4, "int8", "uint",
         False, False, -1, False, False, False, False, "nt", "rescale"),
        (1, 1024, 1024, "float16", "float16", "float16", 4, "int8", "uint",
         False, False, -1, True, False, False, False, "nt", "rescale"),
        (1, 1024, 1024, "float16", "float16", "float16", 4, "int8", "int",
         False, False, -1, False, False, False, False, "nt", "rescale"),
        (1, 1024, 1024, "float16", "float16", "float16", 4, "int8", "int",
         False, False, -1, True, False, False, False, "nt", "rescale"),
        (1, 1024, 1024, "float16", "float16", "float16", 2, "int8", "int",
         False, False, -1, True, False, False, False, "nt", "rescale"),
        (1, 1024, 1024, "float16", "float16", "float16", 2, "int8", "int",
         True, False, -1, True, False, False, False, "nt", "rescale"),
        (1, 1024, 1024, "float16", "float16", "float16", 2, "int8", "int",
         True, False, 128, True, False, False, False, "nt", "rescale"),
        (1, 1024, 1024, "float16", "float16", "float16", 2, "int8", "uint",
         True, True, 128, False, False, False, False, "nt", "rescale"),
        (1, 1024, 4096, "float16", "float16", "float16", 2, "int8", "uint",
         True, True, 128, True, False, False, False, "nt", "rescale"),
        (1024, 1024, 1024, "float16", "float16", "float16", 2, "int8", "int",
         True, False, 128, False, False, False, False, "nt", "rescale"),
        (1024, 1024, 1024, "float16", "float16", "float16", 2, "int8", "int",
         True, False, 128, False, False, False, True, "nt", "rescale"),
        (1024, 1024, 1024, "float16", "float16", "float16", 2, "int8", "int",
         True, False, 128, False, False, True, True, "nt", "rescale"),
        (1024, 1024, 1024, "float16", "float16", "float16", 2, "int8", "int",
         True, False, 128, False, False, True, True, "nt", "original"),
        ([1, 1024
          ], 1024, 1024, "float16", "float16", "float16", 4, "int8", "uint",
         False, False, -1, False, False, False, False, "nt", "original"),
    ],
)
def test_matmul_dequantize_torch_forward(M, N, K, in_dtype, out_dtype,
                                         accum_dtype, bit, storage_dtype,
                                         source_format, with_scaling,
                                         with_zeros, group_size, fast_decoding,
                                         with_bias, propagate_a, propagate_b,
                                         layout, zeros_type):
    import torch
    ...
    if source_format == "uint":
        inputs.append(
            torch.randint(0, maxq, weight_shape, dtype=torch.int8).cuda())
    elif source_format == "int":
        inputs.append(
            torch.randint(-maxq, maxq, weight_shape, dtype=torch.int8).cuda())
    else:
    ...
    if with_scaling:
        ...
        permuted_inputs.append(
            torch.ones([N, K // group_size], dtype=torch.float16).cuda())
    if with_zeros:
        permuted_inputs.append(
            torch.ones([N, K // group_size], dtype=torch.float16).cuda() *
            zeros)
...
@pytest.mark.parametrize(
    ...
    [
        (16, 768, 768, "float16", "float16", "float16", 4, "int8", "uint",
         False, False, -1, True, False, "nt", "original"),
        (16, 768, 768, "float16", "float16", "float16", 4, "int8", "uint",
         False, True, -1, True, True, "nt", "original"),
        (16, 3072, 768, "float16", "float16", "float16", 4, "int8", "uint",
         False, False, -1, True, False, "nt", "original"),
        (16, 768, 3072, "float16", "float16", "float16", 4, "int8", "uint",
         False, False, -1, True, False, "nt", "original"),
    ],
)
def test_matmul_dequantize_propgate_comparison(M, N, K, in_dtype, out_dtype,
                                               accum_dtype, bit, storage_dtype,
                                               source_format, with_scaling,
                                               with_zeros, group_size,
                                               fast_decoding, with_bias,
                                               layout, zeros_type):
    import torch
    torch.random.manual_seed(0)
    ...
    propagated_inputs.append(
        torch.zeros(output_shape, dtype=torch.float16).cuda())
