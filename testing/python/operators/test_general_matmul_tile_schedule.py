...
def assert_dequantize_correctness_with_ladder_ldmatrix_propagate(
    ...
    with tvm.transform.PassContext(config={"tir.use_async_copy": True, "tir.merge_static_smem": False, "tir.disable_cse_tir": True}):
    ...
    if fast_decoding:
        ...
        tvm_transformed_b = tvm.nd.array(torch_transformed_b_lop3.view(torch.int8).view(tvm_transformed_b.shape).numpy())
...
def test_assert_dequantize_correctness_with_ladder_ldmatrix_propagate():
    assert_dequantize_correctness_with_ladder_ldmatrix_propagate(M=256, N=256, K=256, in_dtype="float16", out_dtype="float16", accum_dtype="float16", bit=4, storage_dtype="int8", source_format="uint", with_scaling=True, with_zeros=False, group_size=-1, fast_decoding=False, with_bias=False, layout="nt", zeros_mode="original")
    assert_dequantize_correctness_with_ladder_ldmatrix_propagate(M=256, N=256, K=256, in_dtype="float16", out_dtype="float16", accum_dtype="float16", bit=4, storage_dtype="int8", source_format="uint", with_scaling=True, with_zeros=False, group_size=32, fast_decoding=False, with_bias=False, layout="nt", zeros_mode="original")
    assert_dequantize_correctness_with_ladder_ldmatrix_propagate(M=256, N=256, K=256, in_dtype="float16", out_dtype="float16", accum_dtype="float16", bit=4, storage_dtype="int8", source_format="uint", with_scaling=True, with_zeros=False, group_size=-1, fast_decoding=True, with_bias=False, layout="nt", zeros_mode="original")
