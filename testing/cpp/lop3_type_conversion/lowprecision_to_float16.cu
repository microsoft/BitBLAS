...
REGISTER_GLOBAL_DEVICE_INVOKER(kernelWrapper_i4u_to_f16_scale_zeros, decode_i4u_to_f16_scale_zeros)
REGISTER_GLOBAL_DEVICE_INVOKER(kernelWrapper_i2u_to_f16_scale_zeros, decode_i2u_to_f16_scale_zeros)
REGISTER_GLOBAL_DEVICE_INVOKER(kernelWrapper_i1u_to_f16_scale_zeros, decode_i1u_to_f16_scale_zeros)
...
TEST(DecodeTest, DecodeUInt4ToFloat16WithScalingWithZeros)
{
    constexpr int nbits = 4;
    constexpr int N = 32 / nbits;
    constexpr int QN = N / 8 * nbits;
    ...
    cudaCheckLastError(cudaDeviceSynchronize());
    kernelWrapper_i4u_to_f16_scale_zeros<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(ins_gpu, decoded_gpu, scale_gpu, zeros_gpu);
    cudaCheckLastError(cudaDeviceSynchronize());
    cudaCheckLastError(cudaMemcpy(decoded, decoded_gpu, N * sizeof(half), cudaMemcpyDeviceToHost));
    cudaCheckLastError(cudaFree(ins_gpu));
    cudaCheckLastError(cudaFree(decoded_gpu));
...
TEST(DecodeTest, DecodeUInt2ToFloat16WithScalingWithZeros)
{
    constexpr int nbits = 2;
    constexpr int N = 32 / nbits;
    constexpr int QN = N / 8 * nbits;
    ...
    cudaCheckLastError(cudaMemcpy(zeros_gpu, zeros, 1 * sizeof(half), cudaMemcpyHostToDevice));
    cudaCheckLastError(cudaDeviceSynchronize());
    kernelWrapper_i2u_to_f16_scale_zeros<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(ins_gpu, decoded_gpu, scale_gpu, zeros_gpu);
    kernelWrapper_i2u_to_f16_scale_zeros<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(ins_gpu + QN / 2, decoded_gpu + N / 2, scale_gpu, zeros_gpu);
    cudaCheckLastError(cudaDeviceSynchronize());
    cudaCheckLastError(cudaMemcpy(decoded, decoded_gpu, N * sizeof(half), cudaMemcpyDeviceToHost));
    cudaCheckLastError(cudaFree(ins_gpu));
    cudaCheckLastError(cudaFree(decoded_gpu));
...
TEST(DecodeTest, DecodeUInt1ToFloat16WithScalingWithZeros)
{
    constexpr int nbits = 1;
    constexpr int N = 32 / nbits;
    constexpr int QN = N / 8 * nbits;
    ...
    cudaCheckLastError(cudaMemcpy(zeros_gpu, zeros, 1 * sizeof(half), cudaMemcpyHostToDevice));
    cudaCheckLastError(cudaDeviceSynchronize());
    kernelWrapper_i1u_to_f16_scale_zeros<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(ins_gpu, decoded_gpu, scale_gpu, zeros_gpu);
    kernelWrapper_i1u_to_f16_scale_zeros<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(ins_gpu + QN / 4, decoded_gpu + N / 4, scale_gpu, zeros_gpu);
    kernelWrapper_i1u_to_f16_scale_zeros<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(ins_gpu + QN / 2, decoded_gpu + N / 2, scale_gpu, zeros_gpu);
    kernelWrapper_i1u_to_f16_scale_zeros<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(ins_gpu + QN / 2 + QN / 4, decoded_gpu + N / 2 + N / 4, scale_gpu, zeros_gpu);
    cudaCheckLastError(cudaDeviceSynchronize());
    cudaCheckLastError(cudaMemcpy(decoded, decoded_gpu, N * sizeof(half), cudaMemcpyDeviceToHost));
    cudaCheckLastError(cudaFree(ins_gpu));
    cudaCheckLastError(cudaFree(decoded_gpu));
