...
// __device__ void decode_i1s_to_i8s(int *_i1s, int *_i8s)
// {
...
//                : "=r"(i8s[0])
//                : "r"(i1s), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
//   asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
//                : "=r"(i8s[1])
//                : "r"(i1s >> 2), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
//   asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
//                : "=r"(i8s[2])
//                : "r"(i1s >> 4), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
//   asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
//                : "=r"(i8s[3])
//                : "r"(i1s >> 6), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
//   asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
//                : "=r"(i8s[4])
//                : "r"(i1s >> 8), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
//   asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
//                : "=r"(i8s[5])
//                : "r"(i1s >> 10), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
//   asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
//                : "=r"(i8s[6])
//                : "r"(i1s >> 12), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
//   asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
//                : "=r"(i8s[7])
//                : "r"(i1s >> 14), "n"(BOTTOM_MASK), "n"(I4s_TO_I8s_MAGIC_NUM), "n"(immLut));
// }

__device__ void printBinary(int n)
{
...
extern "C" __global__ void main_kernel0(int8_t *__restrict__ B, int8_t *__restrict__ B_1, const int N = 32)
{
  // print B
  for (int i = 0; i < N / 8; i++)
  {
  ...
  B_1_local[0] = B[0];
  B_1_local[1] = B[1];
  decode_i1s_to_i8s_l16(B_1_local, B_1, N/2);
  B_1_local[0] = B[2];
  B_1_local[1] = B[3];
  decode_i1s_to_i8s_l16(B_1_local, B_1 + N / 2, N/2);
  for (int i = 0; i < N; i++)
  {
    printf("B_1[%d] = %d\n", i, int(B_1[i]));
  }
...
void general_interleave_int8_n16(int8_t *origin_arr, int8_t *interleaved, const int nbit, size_t size_in_bytes, bool verbose = false)
{
  // For fp16 example
  // i4s        {e7,e6,e5,e4,e3,e2,e1,e0}
  //            |-8b-||-8b-||-8b-||-8b-|
...
{
  const int N = 32;
  int8_t *i1s = new int8_t[N];
  for (int i = 0; i < N; i++)
  {
    i1s[i] = rand() % 2;
  }
  // compressed_int8: compress 16 i1s to 2 i8s
  int8_t *i8s = new int8_t[N / 8];
  for (int i = 0; i < N / 8; i++)
  {
    for (int j = 0; j < 8; j++)
    {
  ...
  int8_t *interleaved = new int8_t[N / 8];
  general_interleave_int8_n16(i8s, interleaved, 1, N / 8 * sizeof(int8_t));

  printf("before interleave: %x ", reinterpret_cast<int *>(i8s)[0]);
  printBinary_cpu(reinterpret_cast<int *>(i8s)[0]);
  printf("after interleave: %x ", reinterpret_cast<int *>(interleaved)[0]);
  printBinary_cpu(reinterpret_cast<int *>(interleaved)[0]);

  int8_t *B_local_decode = new int8_t[N];
  int8_t *i8s_gpu;
  int8_t *B_local_decode_gpu;

  cudaMalloc((void **)&i8s_gpu, N / 8 * sizeof(int8_t));
  cudaMalloc((void **)&B_local_decode_gpu, N * sizeof(int8_t));
  // cudaMemcpy(i8s_gpu, i8s, N / 8 * sizeof(int8_t), cudaMemcpyHostToDevice);
  cudaMemcpy(i8s_gpu, interleaved, N / 8 * sizeof(int8_t), cudaMemcpyHostToDevice);
  ...
  main_kernel0<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(i8s_gpu, B_local_decode_gpu, N);
  // print error
