...
__device__ void decode_i2s_to_f16_n8(T1 *_i2s, T2 *B_local_decode, const int N = 8)
{
...
extern "C" __global__ void main_kernel0(int8_t *__restrict__ B, half *__restrict__ B_1, const int N = 16)
{
  // print B
  // print B
  for (int i = 0; i < N / 4; i++)
  ...
  // four fp16xint2b, can decode to 4 fp16
  // half B_1_local[2];
  // B_1_local[0] = B[0];
  // B_1_local[1] = B[1];
  decode_i2s_to_f16_n8(B, B_1, N / 2);
  // B_1_local[0] = B[2];
  // B_1_local[1] = B[3];
  decode_i2s_to_f16_n8(B + 2, B_1 + N / 2, N / 2);
...
void general_interleave_fp16_n8_2b(int8_t *origin_arr, int8_t *interleaved, const int nbit, size_t size_in_bytes, bool verbose = false)
{
  // For fp16 example
  ...
  */
  ...
  for (int idx = 0; idx < size; ++idx)
  {
    ...
    // design for n8 case
    // convert {e15,e13,e11,e9,e7,e5,e3,e1,e14,e12,e10,e8,e6,e4,e2,e0}
    // convert {e15,e13,e11,e9,e7,e5,e3,e1,e14,e12,e10,e8,e6,e4,e2,e0}
...
// nvcc -gencode arch=compute_80,code=sm_80 -lineinfo -O3 fast_decode_s2_fp16_n8.cu -o fast_decode_s2_fp16_n8; ./fast_decode_s2_fp16_n8
int main()
{
  const int nbits = 2;
  ...
  general_interleave_fp16_n8_2b(is, interleaved, nbits, 4 * sizeof(int8_t), true);
  for (int i = 0; i < 4; i++)
  {
    printf("interleaved[%d] = %d\n", i, (int)interleaved[i]);
  }
