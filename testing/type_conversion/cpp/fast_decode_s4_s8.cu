...
extern "C" __global__ void main_kernel0(int8_t *__restrict__ B, int8_t *__restrict__ B_1)
{
  ...
  printf("B[2] = %d\n", int(B[2 + 4]));
  printf("B[3] = %d\n", int(B[3 + 4]));
  decode_i4s_to_i8s(reinterpret_cast<int *>(B), reinterpret_cast<int *>(B_1));
  printf("B_1[0] = %d\n", (B_1[0]));
  printf("B_1[1] = %d\n", (B_1[1]));
  printf("B_1[2] = %d\n", (B_1[2]));
  printf("B_1[3] = %d\n", (B_1[3]));
...
// nvcc -gencode arch=compute_80,code=sm_80 -lineinfo -O3 fast_decode_s4_s8.cu -o fast_decode_s4_s8; ./fast_decode_s4_s8

int main()
{
  // create four int8_t values
  int8_t *i4s = new int8_t[16];
  i4s[0] = 1;
  i4s[1] = 2;
  i4s[2] = 3;
  i4s[3] = 4;
  ...
  int8_t *i8s = new int8_t[8];
  for (int i = 0; i < 8; i++)
  {
    i8s[i] = (i4s[2 * i + 1] << 4) | i4s[2 * i];
  }
  ...
  cudaMemcpy(i8s_gpu, interleaved, 8 * sizeof(int8_t), cudaMemcpyHostToDevice);
