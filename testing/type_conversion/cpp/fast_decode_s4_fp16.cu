...
template <typename T1, typename T2>
__device__ void decode_i4u_to_f16(T1 *_i4s, T2 *B_local_decode, const int N = 8)
{
  decode_i4b_to_f16<T1, T2, false>(_i4s, B_local_decode, N);
}
...
extern "C" __global__ void main_kernel0(int8_t *__restrict__ B, half *__restrict__ B_1, const int N = 8)
{
  ...
  decode_i4u_to_f16(B, B_1);
  // decode_i4s_to_f16(reinterpret_cast<int *>(B), B_1);
  __syncthreads();
  for (int i = 0; i < N; i++)
  {
    printf("B_1[%d] = %f\n", i, float(B_1[i]));
...
// nvcc -gencode arch=compute_80,code=sm_80 -lineinfo -O3 fast_decode_s4_fp16.cu -o fast_decode_s4_fp16; ./fast_decode_s4_fp16
int main()
{
  const int N = 8;
  const int nbits = 4;
  // create four int8_t values
  int8_t i4_data[8] = {1, 2, 3, 4, 11, 12, 13, 14};
  int8_t *i4s = new int8_t[4];
  general_compress(i4_data, i4s, 4, 8);
  for (int i = 0; i < 4; i++)
  {
      printf("%d ", i4s[i]);
  }
  int8_t *interleaved = new int8_t[N / 2];
  general_interleave_fp16(i4s, interleaved, nbits, N / 2 * sizeof(int8_t), true);

  half *B_local_decode = new half[8];
  int8_t *i4s_gpu;
  half *B_local_decode_gpu;

  cudaMalloc((void **)&i4s_gpu, 4 * sizeof(int8_t));
  cudaMalloc((void **)&B_local_decode_gpu, 8 * sizeof(half));
  cudaMemcpy(i4s_gpu, interleaved, 4 * sizeof(int8_t), cudaMemcpyHostToDevice);
  cudaMemcpy(B_local_decode_gpu, B_local_decode, 8 * sizeof(half), cudaMemcpyHostToDevice);
  // print the last error
  cudaError_t cudaerr = cudaDeviceSynchronize();
  if (cudaerr != cudaSuccess)
    printf("kernel launch failed with error \"%s\".\n",
           cudaGetErrorString(cudaerr));
  main_kernel0<<<dim3(1, 1, 1), dim3(1, 1, 1)>>>(i4s_gpu, B_local_decode_gpu);
  // print error
  cudaerr = cudaDeviceSynchronize();
  if (cudaerr != cudaSuccess)
    printf("kernel launch failed with error \"%s\".\n",
           cudaGetErrorString(cudaerr));
  cudaMemcpy(B_local_decode, B_local_decode_gpu, 8 * sizeof(half), cudaMemcpyDeviceToHost);
