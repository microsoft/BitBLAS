...
# Your existing code for the operations, etc.
...
# Parametrized fixture for all test cases
@pytest.fixture(params=c_lists + m_lists + e_lists + r_lists + d_lists + p_lists,
                ids=[item[0] for item in c_lists + m_lists + e_lists + r_lists + d_lists + p_lists])
def test_case(request):
    return request.param

# Test function
def test_operations(arch, code_generator, test_case):
    ...
    for config in configs:
        cpresult = code_generator.compile(output_nodes, config, "cuda", kernel_name="Fused")
