    ...
    for rect in rec:
        ...
        if height == 0:
            if label == "TensorIR":
                ...
                if "vLLM" in label:
                    warning_text = f"vLLM-4b Not Support"
                else:
    ...
    for rect in rec:
        ...
        if height == 0:
            if label == "TensorIR":
                ...
                if "vLLM" in label:
                    warning_text = f"vLLM OOM"
                elif "ONNX" in label:
                    warning_text = f"ONNX Fail"
                else:
                    warning_text = f"{label} OOM"
            ax2b.text(
                rect.get_x() + rect.get_width() / 2,
                height + 0.05,
                warning_text,
...
# 在这个ax2_2这个图上面加一个Out of memory的标记
ax2_2.text(
    0.5,
    0.5,
    "Out of memory",
...
# 保存图形
plt.savefig(
    "pdf/figure9_end2end_v100.pdf",
    bbox_inches="tight",
)
plt.savefig(
    "png/figure9_end2end_v100.png",
    bbox_inches="tight",
    transparent=False,
    dpi=255,    
)
